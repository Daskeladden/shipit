...
find_emacs_package() {
    ...
    for dir in \
        ...
        ~/.config/emacs/elpaca/repos/"$package" \
        ~/.emacs.d/elpaca/repos/"$package"; do
        if [ -d "$dir" ]; then
            echo "$dir"
            return 0
        fi
...
if REQUEST_DIR=$(find_emacs_package "request"); then
    LOAD_PATH="$LOAD_PATH -L $REQUEST_DIR"
fi
...
if [ $COMPILE_EXIT_CODE -eq 0 ] && [ "$WITH_NATIVE_COMPILE" = true ]; then
    ...
    if [ "$NATIVE_COMPILE_AVAILABLE" = true ]; then
        ...
        emacs -batch $LOAD_PATH -f batch-native-compile lisp/*.el
...
if [ $COMPILE_EXIT_CODE -eq 0 ] && [ $NATIVE_COMPILE_EXIT_CODE -eq 0 ]; then
    ...
    emacs -batch $LOAD_PATH -l ert -l load-shipit.el -l test/test-shipit.el -f ert-run-tests-batch-and-exit >&2
    TEST_EXIT_CODE=$?
