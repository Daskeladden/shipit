      ...
      (let ((to-mark '()))
        ...
        (dolist (a to-mark)
          (let ((number (or (cdr (assq 'number a))
                ...
                (reason (cdr (assq 'reason a))))
            (when (and number repo)
              (shipit--debug-log "Auto-mark: marking %s/%s/%s (reason=%s)"
                                 repo type number reason)
              (shipit--mark-notification-read number repo t type)
              (cl-incf count))))
        (when (> count 0)
          (shipit--debug-log "Auto-mark: marked %d activit%s"
                             count (if (= count 1) "y" "ies")))))
    count))

(defun shipit-notifications-diagnose ()
  "Print a quick diagnostic of the notifications state.
Shows the activity hash by source/repo, recent locally-marked-read
entries, the active auto-mark rules and -- for each registered Jira
config -- the cached account-id used by involvement detection.
Use this when notifications you expect to see are missing."
  (interactive)
  (require 'shipit-issue-backends)
  (let ((buf (get-buffer-create "*shipit-notifications-diagnose*")))
    (with-current-buffer buf
      (let ((inhibit-read-only t))
        (erase-buffer)
        (special-mode)
        (insert "Activity hash, by repo:\n")
        (let ((by-repo (make-hash-table :test 'equal)))
          (when (and (boundp 'shipit--notification-pr-activities)
                     shipit--notification-pr-activities)
            (maphash (lambda (_k a)
                       (let* ((source (or (cdr (assq 'source a)) 'github))
                              (repo (cdr (assq 'repo a)))
                              (k (format "%s (%s)" repo source)))
                         (puthash k (1+ (gethash k by-repo 0)) by-repo)))
                     shipit--notification-pr-activities))
          (let ((rows '()))
            (maphash (lambda (k v) (push (cons k v) rows)) by-repo)
            (dolist (row (sort rows (lambda (a b) (string< (car a) (car b)))))
              (insert (format "  %s: %d\n" (car row) (cdr row))))))
        (insert (format "\nAuto-mark rules (%d):\n"
                        (length (and (boundp 'shipit-notifications-auto-mark-read-rules)
                                     shipit-notifications-auto-mark-read-rules))))
        (dolist (rule (and (boundp 'shipit-notifications-auto-mark-read-rules)
                           shipit-notifications-auto-mark-read-rules))
          (insert (format "  %S\n" rule)))
        (insert (format "\nLocally-marked-read entries: %d\n"
                        (if (and (boundp 'shipit--locally-marked-read-notifications)
        ...
        (when (and (boundp 'shipit--locally-marked-read-notifications)
                   (hash-table-p shipit--locally-marked-read-notifications))
          (let ((keys '()))
            (maphash (lambda (k _v) (push (format "%s" k) keys))
                     shipit--locally-marked-read-notifications)
            (dolist (k (sort keys #'string<))
              (insert (format "  %s\n" k)))))
        (insert "\nJira account-id cache (used by involvement detection):\n")
        (when (and (featurep 'shipit-issue-jira)
                   (boundp 'shipit-issue-jira--myself-cache))
          (maphash (lambda (base-url id)
                     (insert (format "  %s -> %s\n" base-url (or id "<nil>"))))
                   shipit-issue-jira--myself-cache))
        (insert "\nJira backends configured (one /myself fetch per row):\n")
        (when (boundp 'shipit-issue-repo-backends)
          (dolist (entry shipit-issue-repo-backends)
            (let* ((cfg (cdr entry))
                   (backend-id (plist-get cfg :backend)))
              (when (eq backend-id 'jira)
                (let ((my-id (and (fboundp 'shipit-issue-jira--fetch-myself)
                                  (shipit-issue-jira--fetch-myself cfg))))
                  (insert (format "  %s (display=%s) -> account-id=%s\n"
                                  (plist-get cfg :base-url)
                                  (plist-get cfg :display-name)
                                  (or my-id "<nil>")))))))))
      (goto-char (point-min)))
    (display-buffer buf)))
